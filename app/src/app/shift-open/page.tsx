    ...
    <PosFrame kicker="STALL OS · SHIFT" title="Open shift">
      <div className="flex flex-col gap-3">
        <label className="flex flex-col gap-1.5">
          <PanelLabel>Shift name</PanelLabel>
          <Field value={name} onChange={(e) => setName(e.target.value)} />
        </label>
        <label className="flex flex-col gap-1.5">
          <PanelLabel>Venue</PanelLabel>
          <Field value={venue} onChange={(e) => setVenue(e.target.value)} placeholder="e.g. School quad" />
        </label>
        <label className="flex flex-col gap-1.5">
          <PanelLabel>Event (multi-day, optional)</PanelLabel>
          <Field value={eventName} onChange={(e) => setEventName(e.target.value)} />
        </label>
        <label className="flex flex-col gap-1.5">
          <PanelLabel>Opening float (₹)</PanelLabel>
          <Field type="number" value={float} onChange={(e) => setFloat(e.target.value)} />
        </label>
        <div className="border-2 border-ink bg-white p-3 flex items-center justify-between gap-2.5">
          <div>
            ...
            <div className="text-[13px] text-neutral-600 max-w-[26ch]">
              {pressOnSite
          ...
          <button
            ...
            className={`border-2 border-ink px-3 py-2 font-extrabold text-xs min-h-[44px] ${
