...
/** A `<select>` that lines up with `Field`.
 *
 *  There is no Select in `ui.tsx` and the primitives file is frozen for this
 *  pass, so this lives here rather than being re-hand-rolled in six admin
 *  pages with six different heights — which is exactly what it was before.
 *  It types no hex and no arbitrary size; it borrows Field's geometry. */
export function AdminSelect({
  ...
  label: string;
  /** Only for a control INSIDE a table cell, where the column head is already
   *  the visible label and repeating it on every row is noise rather than
   *  help. Never on a form. */
  labelHidden?: boolean;
  hint?: string;
}) {
  return (
    <div className={clsx("flex flex-col gap-[var(--space-1)]", className)}>
      <label htmlFor={id} className={clsx(labelHidden ? "sr-only" : "t-label text-[var(--color-ink)]")}>
        {label}
      </label>
      <select
        {...rest}
        id={id}
        className={clsx(
          "min-h-[var(--tap-admin)] w-full rounded-[var(--radius-md)] border-[3px] border-[var(--color-ink)]",
          "bg-white px-3 t-sm font-semibold",
          "transition-[background-color] duration-[var(--dur-fast)] ease-[var(--ease-out)]",
          "focus:bg-[var(--color-yellow-wash)]"
        )}
      >
        {children}
      </select>
      {hint && <p className="t-sm text-[var(--color-muted)]">{hint}</p>}
    </div>
  );
}
