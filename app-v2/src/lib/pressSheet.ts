 ...
 *
 *  Ported from v1 (`app/src/lib/pressSheet.ts`) unchanged except that the
 *  print-area type is app-v2's shared `PrintArea` (`lib/domain/types.ts`)
 *  rather than a locally redeclared one — same shape (x/y/w/h fractions of
 *  the mockup plus cm_w/cm_h), so this ports directly. */
