-- version: 20260811182016
-- applied name: stall_010_prep_order_rpc
...
begin
  ...
  loop
    update stall_product_skus
     ...
     where id = v_row.product_sku_id and stock_qty - v_row.qty >= 0;
    ...
    values ('product', v_row.product_sku_id, -v_row.qty, 'sale', p_order_id, p_actor, v_order.environment_id);
  end loop;
  ...
  loop
    update stall_sticker_designs
       set stock_qty = stock_qty - v_row.qty
     where id = v_row.sticker_design_id and stock_qty - v_row.qty >= 0;
    get diagnostics v_hit = row_count;
    if v_hit = 0 then
    ...
    insert into stall_inventory_movements (sku_type, sku_id, delta, reason, ref_order, actor, environment_id)
    values ('sticker', v_row.sticker_design_id, -v_row.qty, 'sale', p_order_id, p_actor, v_order.environment_id);
  end loop;
